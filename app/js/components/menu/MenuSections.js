            ...
            {category.sections.map(function(section, index) {
              ...
              return (
                <li className="full-section" key={section.id} data-order={section.order} style={ currentSectionStyle }>
                  <Section key={ category.title + section.title } isAdmin={isAdmin} section={section} categoryId={this.props.category.id} mouseDown={this.mouseDown} dragEnd={this.dragEnd} dragStart={this.dragStart}/>
                </li>
              );
            }.bind(this))}
          </ul>
          <NewSection isAdmin={isAdmin} sections={category.sections} categoryId={category.id} />
        </div>
      );
    } else {
      var placeholder;
      if (isVisible) {
        placeholder = <NewSection isAdmin={isAdmin} sections={category.sections} categoryId={category.id} />;
      }
      return (
        <div>
          {placeholder}
